...
- スマートコントラクトとはブロックチェーン上で動くプログラムである
...
- Ethereumは「**ワールドコンピューター**」である
  - トランザクションをプログラム化しブロックチェーン上でも扱えるようになれば、電卓からコンピュータに進歩したような大きな変化を、ブロックチェーン上の世界でも実現できるのでないか
...
- 分散型アプリケーション（Decentralized Applications; DApps）
  ...
  - 分散型アプリケーションとは、Google Driveなどのような誰か大企業が管理しているアプリではなく、管理者が不在のアプリケーションである
　アプリケーションの改変などのコントロールができないもの
- プログラムの実行や、その実行履歴・実行結果の管理をどこかの誰かがやってくれる
- 運用主体が存在しないため、ユーザーはその誰かを信用する必要がない！
...
- EOA to CA (Message Call) 既存のコントラクトの引数にデータを送り、関数であるプログラムを実行
- CA to CA (*Internal Transaction) 
  - CAが (プログラムに従って) 別のCAにETHを送金したり、新たなCAを作成する場合もある。（例：トークンバウンドアカウント（ERC-6551））
...
- 一般化の目的は、ワールドコンピューターを作りたい、その上で動くアプリケーション開発を行えるようにするため
