...
- データをどのように L1 に送るかの仕組みには 2 種類ある
  ...
  - calldata を使用するものと blob というものを使用するやり方である
  - 前半は calldata を使用した仕組みについて取り上げ、blob を使用したものは後半で取り上げる
...
- L2 ブロックはシーケンサーが`ArbitrumInternalTxType`という種類のトランザクションを挟むことで区切られる
