...
- Arbitrum は Ethereum の L2 チェーンである
  ...
  - Ethereum との TPS 等の比較は[ここ](https://chainspect.app/compare/arbitrum-vs-ethereum)で確認できる
...
- Arbitrum One は「Optimistic Rollup」を採用している
  ...
  - Rollup の中でも、L1 に送信されるデータが適切なルールに従っていると「楽観的に想定」したものを「Optimistic Rollup」と呼ぶ
    ...
    - つまり L1 に記録するデータがすべて正しいと仮定するということ
      <!-- 正しくない可能性があるから楽観的と言ってると思っていたが・・？ -->
      <!-- Re：最終的には正しい状態を残さないといけません。チャレンジ期間が終了した後のデータはすべて正しい必要があります。間違いが残ってしまうようなことがあれば、それはもはやプロトコルが成り立っていないことになります。 -->
...
- ## シーケンサー
  ...
  - 現在シーケンサーは Offchain Labs によって実行されている
    <!-- 特定の企業？ってことは今のところ超トラスト入ってるな・・．まあ，信用できなければDelayed Inbox（コントラクト）経由でTx起こせるみたいだが．． -->
    <!-- Re：その通りです。Arbitrumの課題はそこにあります。ただトラストを受ける以上はこの企業にも法的責任があります。 -->
...
- ちなみに L2 ブロックは「Arbitrum ブロック」と書いたが「Nitro ブロック」と書いてあるところもある（面倒なので L2 ブロックでいく）
...
1. ユーザーがトランザクションをインボックスに送信
   <!-- インボックスってどこだろう？ Sequencerに単に送信する，ということだと理解．-->
   <!-- Re：その理解で大丈夫です。実際のところ送り先は SequencerとAggregatorとForwarderの三種類あるのですが、別に特別なこともしておらず、混乱するだけなのでインボックスとしています。厳密には、ユーザーはForwarderかSequencerに直接トランザクションを送信します（Aggregatorは見たところユーザーが直接トランザクションを送ることはできなさそう、Forwarder経由と思われる）。ForwarderはSequencerまたはAggregatorにL2トランザクションを転送し、AggregatorはDelayed InboxにバッチでL2トランザクションを投稿します。どちらにしろSequencer経由のSequencer Inbox行きか、Aggregator（またはユーザーが直接）によるDelayed Inbox経由のSequencer Inbox行きです。-->
2. シーケンサーがトランザクションを順序付けし（無効なトランザクションは破棄）、ローカルでトランザクションを実行する
   <!-- シーケンサーはTxを実行するのは，有効かどうか年のために実行するだけで特にステートの変更はしないという理解でOK？ -->
   <!-- Re：あっています。有効か確認するのと同時に、トランザクションの実行結果を即座に返します（ソフトファイナリティ）。 -->
3. 十分なトランザクションがたまったら、シーケンサーはデータを圧縮し calldata として [Sequencer Inbox](https://etherscan.io/address/0x1c479675ad559DC151F6Ec7ed3FbF8ceE79582B6) に投稿（L1 コントラクト）
   <!-- 複数のArbitrumのTxをcalldataに入れたL1へのTxを生成し，Sequencer InboxというL1コントラクトを実行． -->
   <!-- Re：あっています。 -->
4. バリデーターは 3 で投稿されたトランザクションを実行する
5. バリデーターはローカルでステートを更新（L2 ブロックを作成）する
   <!-- Sequencer Inbox内のTxから順番にいくつか選びL2ブロックを生成するってこと？ -->
   <!-- Re：L2ブロックを作成するのは Sequencer（ArbOS）なので、ただ単に順番通りにいくつか取得するだけだとおもいます。L2ブロックを区切るためにSequencer（ArbOS）が挿入した「Start Block」トランザクションが含まれた状態で、L1に投稿されています。ここでL2ブロックを作成、と書いているのはSequencerにステートを更新する権限がないから、必然的にバリデーターの役割になるからです。 -->
...
- 2、3 それぞれのステップでレシートがひとつずつ返される
  ...
  |        | 名称                                      | 説明                                                                                                        |
  | ------ | ----------------------------------------- | ----------------------------------------------------------------------------------------------------------- |
  | １回目 | ソフトファイナリティ（Soft Confirmation） | L1 に投稿する前に返される、シーケンサーからの実行結果。シーケンサーに悪意があれば間違っている可能性もある。 |
  | ２回目 | ハードファイナリティ（Hard Confirmation） | L1 でバッチデータが確認された後に返される、信頼できる実行結果。                                             |

  <!-- これは図で言うSoft confirmation, Hard confirmationのこと？用語は統一したほうがよい． -->
  <!-- Re：直感的にファイナリティのほうがわかりやすいかなと思い、日本語ではファイナリティとしています。括弧で補足しておきます。 -->

  - ソフトファイナリティ（Soft Confirmation）は、実行結果を即座にユーザーに返すために存在する
    ...
    <!-- ソフトファイナリティのレシートがあるんだろうけど，それもここで改めて記述したほうが良さそう． -->
    <!-- Re：追記します。 -->

    - ソフトファイナリティでもレシートが返される

  - 逆にハードファイナリティ（Hard Confirmation）は、正常なトランザクションとして処理が完了したことを表す
...
- シーケンサーは L1 の Sequencer Inbox に以下のデータを送る
  <!-- これが上で言っているL2ブロックのこと？ -->
  <!-- Re：dataがL2ブロックに当たります。そもそもL2ブロック自体ただのトランザクションの塊ですが（もはやブロックではない）。 -->
...
- バリデータは L1 の Rollup に以下のデータを calldata として送る
  <!-- L1のコントラクトに の間違い？あとこのデータにTxのデータが含まれてない気が・・？
      あと，ここで書いてある「ノード」とはL2ブロック，という解釈をすればよい？
      引数から調べてみたところcreateNewNode()という関数．
      https://github.com/OffchainLabs/nitro-contracts/blob/90037b996509312ef1addb3f9352457b8a99d6a6/src/rollup/RollupCore.sol#L563

      バリデータは，Sequencer indexに記録されているデータをもとに不正なTxが入っていないかをチェックしていると理解している．
      ということは特にコントラクトに情報を記録する必要はないとの理解だが↓の4項目のデータはなんのためにどこに送信している？

      ※ ここの全体的な記述について要確認．
  -->
  <!-- Re：ノードはRBlockのことです。また最初の文章の意味が少しわかりません。RollupというL1コントラクトに以下のデータを送信します。このコントラクト自体にトランザクションデータを送信する必要はありません。おっしゃる通りトランザクションデータはすでにSequencer Inboxによって保持されており、わざわざもう一度L1に上げる必要はありません。最後の値はL1上で計算する値が、バリデーターが送信してきたデータと一致するかどうかを検証するために使用されます（RBlock作成には不要）。上から３つのデータだけでは、バリデーターが同じリプレイバイナリ（正しいwasmRoot）を使用しているか確認できない、という理由で含まれているのもの考えられます。とは言ったものの気休めに過ぎない気がします。不正があってもまだ対応可能な段階なので問題はありませんが。 -->
...
- 上記の送られてきたデータをもとに作成される RBlock の構造は以下の通り
  ...
  | 名称                        | 型      | 詳細                                                                                                                                                                                                              |
  | --------------------------- | ------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
  | stateHash                   | bytes32 | このノード時点のチェーンの状態を表すハッシュ値 <!-- チェーンと言うか，L2の本ブロック時点でのステートルートと思われる． --><!-- Re：確かにそちらのほうが正確ですね。L2チェーンの状態はL1より進んでいますしね。 --> |
...
<!-- このL2ブロックという情報は何？
Sequencer Indexに入っている各々のTx情報とRBlock情報を組み合わせると仮想的な（？）L2の正しいブロックが復元でき，それをL1のブロックと同様に見立ててeth_getBlockByNumber()など互換性のあるコマンドからデータを返しているもの？

 -->
<!-- Re：その理解であっていると思います。承認されたRBlockからL2のステートが作成されるので、そこに新たなトランザクションによるステートを追加していくという流れです。 -->
...
- ちなみに現状バリデーターへの報酬はない（なぜ）
  <!-- Validatorはステーキングしないとなれなくて，ステーキング報酬がある？と思う． -->
  <!-- Re：Arbitrumチームに質問投げていますが、まだ返事が返ってきません。 -->
