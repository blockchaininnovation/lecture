...
- Arbitrum は Ethereum の L2 チェーンである
  ...
  - Ethereum とのTPS等の比較は[ここ](https://chainspect.app/compare/arbitrum-vs-ethereum)で確認できる
...
- ちなみにL2ブロックは「Arbitrum ブロック」と書いたが「Nitro ブロック」と書いてあるところもある（面倒なので L2 ブロックでいく）
...
- 上記の送られてきたデータをもとに作成される RBlock の構造は以下の通り
  ...
  | 名称                        | 型      | 詳細                                                               |
  | --------------------------- | ------- | ------------------------------------------------------------------ |
  | stateHash                   | bytes32 | このノード時点のチェーンの状態を表すハッシュ値      <!-- チェーンと言うか，L2の本ブロック時点でのステートルートと思われる． -->               |
