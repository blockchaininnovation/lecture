...
→ pは255bitだった．
ってことは32バイトの正の整数値は実質的にはp以下の数に変換されKZGコミットメントに使用される．
→ 完全に256bit=32bytesは表現しきれず，約2bit分足らない，ということは4つのデータは同じ有限体上の数となってしまう．
→ 4096個も点があるからそのへん一つ一つのバイトは重複部分があっても全体としてはユニークに表現可能だから大丈夫，という考え方？
→ というか実は32バイトずつに分割されているんじゃなくて位数内で表現可能な254bit単位で分割されている？
...
→ プロトダンクシャーディング時点ではハッシュでも代替できるが，フルダンクシャーディングになったときのデータアベイラビリティサンプリングではKZGコミットメントじゃないとダメ．
→ コミットメントの準同型性があるため，部分データの検証が可能．これはハッシュだとできない．
...
- $u$: 現在のブロックで使用されたガス
- $t$: ターゲットガス使用量
<!-- TARGET_DATA_GAS_PER_BLOCK = 3*(2**17)
を目標にしている．
 -->
