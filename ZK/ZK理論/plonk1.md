...
で，その□と矢印からなる図を下で，コンストレイントシステムとして数式に変換する，と．
 -->
## 算術回路の制約条件
Plonkの証明者は次に、算術回路の充足可能性問題を、ゲートの入出力間の制約を定義するgate constraintsと、一方の出力がもう一方の入力になっている2つのゲート、つまり同じワイヤを共有するゲートの間で同じ値がコピーされていることを要請するcopy contraintsに変換します。
...
同値変換（⇔）である．
なんでこんな変換をしているかというと，後々のKZGコミットメント利用のための多項式に変換するための一つのステップ．
