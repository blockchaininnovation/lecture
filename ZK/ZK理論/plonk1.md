...
ということはその人が乱数と言わず好きな数を選んでもよい？
好きな数を選んでもいいけど，あんまり単純な数にしてしまうと，F=Gでも制約が満たされるx_iとx_σ(i)で適当な値が選べる可能性が出てきてしまい，セキュリティ的に甘くなる．
...
<!-- 
ここまでで制約条件が作られた．
制約条件とは，変数がめちゃめちゃ多い(x_iのiが多い)連立方程式．
あくまで「制約条件」なのでこのたくさんある連立方程式の解はない．

ゲート制約は項数が5の単純な方程式だが，コピー制約だけはながーーーい(ゲート数の3倍の項の数)方程式が1つ．

なので，式の数は単純な項数5の方程式が超いっぱい（ゲート数）と，ながーーーい方程式が1つ．

最適化問題のs.t.のあとの制約条件と同じ意味合い．
...
<!-- 
KZGコミットメントではどうやってもとの「問題」と「解」を取り扱っているんだろう？
「問題」は公開されていないとダメで，解を隠蔽化したい．
...
つーことは全部これを共有したらダメで，解も多項式の係数のなかに含まれている．
制約条件と解も含めたすべての情報が多項式の係数として表現されている．

→ 式変形を見ていくと単純に多項式の係数をうまく取る，というやり方ではないぽい．
...
plonk2のページを見ると，最後の積で表現されている式(**)ではなくその1個上の式(*)でKZGコミットメントを適用している．
(*)が成り立つ⇒(**)が成り立つ
なので
