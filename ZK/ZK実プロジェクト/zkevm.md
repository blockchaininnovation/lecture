...
--  --------------  ----------------------------------   ---------------------------------------
...
53  PUSH1 0x80      [         ,        ,      80]   {40:   ,  80:  ,  a0:         }
54  PUSH1 0x40      [         ,      40,      80]   {40:   ,  80:  ,  a0:         }
56  MSTORE          [         ,        ,        ]   {40: 80,  80:  ,  a0:         }
57  CALLVALUE       [         ,        ,   value]   {40: 80,  80:  ,  a0:         }
58  DUP1            [         ,   value,   value]   {40: 80,  80:  ,  a0:         }
59  PUSH2 0x0010    [   0x0010,   value,   value]   {40: 80,  80:  ,  a0:         }
61  JUMPI           [   0x0010,   value,   value]   {40: 80,  80:  ,  a0:         }
62  REVERT          [   0x0010,   value,   value]   {40: 80,  80:  ,  a0:         }
63  JUMPDEST        [   0x0010,   value,   value]   {40: 80,  80:  ,  a0:         }
64  POP             [         ,   value,   value]   {40: 80,  80:  ,  a0:         }
...
```
...
| :----: | ------------- | ------- | ---- | ----------- |
| `0x40` | `0`           | `Write` |      | 初期化       |
| `0x40` | `0x80`        | `Write` | 0    | `56 MSTORE` |
```

リソースへのアクセスが正しく行われたかをTable形式にして回路へのインプットとする。

## EVM証明

Ethereumの機能に関する証明をTableへのLookupを通して行う。

1. オペコードとバイトコードが対応していること
2. オペコードが正しい順番で実行されたこと
3. トランザクションが正しく署名されていること
4. ブロックの値が正しいこと
5. 記憶領域間のコピーが正しく行われたこと
