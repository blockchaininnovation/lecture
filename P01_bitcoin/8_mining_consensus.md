...
- コンセンサスアルゴリズムの色々
  ...
  - PoW
  - PoS
<!-- TODO 略称は，文の初出のところでは略称じゃないフルの呼び方を添えて記述してください． -->

- 中央管理者なしでの真実と合意の形成

中央集権型のシステムでは、その運営者を信用している例として、Visa の支払いが挙げられる。
<!-- TODO 「Visa」と特定の企業名は出さないようにしてください．中央集権型のシステムで運営者を信用している例というのはどういう状況なのか初見の人もわかる説明も加えてください． -->

しかし、分散化されたコンセンサスにおいては、個々が独立して動いていても全体として最適に動くことが求められる。これはコンセンサスの創発とも呼ばれ、以下の 4 つの相互作用で成り立つ
...
<!-- TODO ここの説明がわかりにくいです．全参加者が検証を常に行うとなぜコンセンサスが創発されるのでしょう？「コンセンサス（合意）を創発」というのはそのままの意味で捉えるととても難しく，これもどういう状況なのか初見の人も分かる説明を加えたいです． -->

  このコンセンサスの方式が、情報の信頼性を保証する別の仕組み、PKI と最も違う点である。
  <!-- TODO PKIとは何なのかの説明も入れてください． -->
...
1. そのトランザクションのインプットを過去に支払ってないこと．二重支払いの防止。支払い済みのインプットから再度支払うことはできない

2. そのインプットの合計額がアウトプットの合計額以上になっていること 新たにビットコインが作成されてないかを確認する(coinbase トランザクションは例外)
<!-- TODO 合計額「以上」について，等しいときはOKですか？また，アウトプットがインプットより大きくなっていることを「 新たにビットコインが作成されてないかを確認す」と表現していると思われますがわかりにくいです．-->
3. ScriptSig が対応するInputの ScriptPubKey のアンロックに成功していること 連結したスクリプトが有効であることを確認する(ほとんどすべてのトランザクションにおいては ScriptSig にある 1 つ以上の署名が有効であると確認することを意味する)
<!-- TODO 「連結した」とは何と何が連結していることを想定していますか？また（）内に記述されている内容も不明瞭です．ScriptSigに2つ以上の署名がある場合はなんなのか，「ほとんどすべて」とはどういうTxなのか，などを感じます．曖昧な表現を正確に書き換えてください． -->
- インプットの支払い状況の確認
<!-- TODO たぶん，ここから「-」で表現されているところは上記の１．２．３に対応しているのかな，と推察しますが，表現が対応されていないため不明瞭です． -->
1. 二重支払い防止のため、ノードは各インプットが存在すること及び過去に使われてないことを確認する(UTXO セットにアクセスし、UTXO を追跡することであるインプットが二重支払いかどうか検証可能)
<!-- TODO UTXOを追跡 → UTXOであるかどうか確認，ということ？ -->
2. UTXO は Unspent Transaction Outputs の略。インプットに未だに払われていないものが入っていなければ、二重支払いであるということが判明する。
<!-- TODO 二重否定は理解するのに混乱します．できればストレートに表現してください． -->
...
- 署名の確認
<!-- TODO　
以下の１から５のプロセスでどうやって署名の検証をしているかがよくわかりません．
より正確に記述するか，処理をざっと解説するかどちらかにしてください．

「署名ハッシュ」とは，署名対象のメッセージのハッシュ値のことかなと想像しています．
そしておそらく，署名対象のMSGを抜き出すときに，署名そのものは除外しないとだめ，という話をしたいと思われます．

が，以下の説明だと，ScriptSig部という署名部分を書き換えて保管しているように読めます．

そもそもコンセンサスの仕組みの一部としての「署名の確認」において，署名対象のハッシュ値の話が必要なのかどうかも含め，文章の校正をお願いします．
 -->
...
その他の補足説明
<!-- TODO 引用元のListが存在していると思われますので，そのURLなどを追記してください．
下の方にまとめて貼ってあるのかもしれませんが，すべてのリンクをクリックして該当の引用元を探す必要がないように，近くに貼ってください． -->
...
- その他のコンセンサス
<!-- TODOここ（独立したトランザクション内部）に書くことに違和感を覚えます．
ビットコインの仕組みではないと明記したうえで他の箇所に書くことを検討ください．
また，これもどこかからの引用の場合は引用元URLを追記してください．
というかPBFTだけ書いてあるのも不自然なので消してしまって良いかも．
-->
  - PBFT - コンソーシアム型むきのコンセンサスアルゴリズム。ノードが小規模
  <!-- TODO ノードが小規模とは？ → ノード数が少ないのか，ブロックチェーンのデータ量が小さいためマシンスペックが小さいノードで構成されているのか？ -->
  
  かつ、固定されている時向け。リーダーが Validating peer
  <!-- TODO 「リーダー」や「Validation peer」などプレイヤーをいきなり書くのではなく，どんなプレイヤーを想定しているのかまず一覧を記述してください． -->
  
   に対して Tx を正しく送ったことを信頼している - 長所 - Tx を実行する権限が、プライマリーノード
   <!-- TODO プライマリーノードとは？上記のリーダーのこと？そうであれば言葉を統一すると誤解が少なくなります． -->
   
   しかないため、トランザクションに分岐が発生せず、履歴が覆ることがない - 短所 - 基本的に、ノードを信頼しないといけない
   <!-- TODO ノードとは？構成されているノードすべて？それともプライマリーノードのみ？ -->
    - 一つの機械、ノードが一つの vote なので、一定数を超えた悪意があるものがいれば、コンセンサスは止まる
    <!-- TODO 一定数を超えた悪意があるものがあれば → 悪意のあるノードが一定数を超えると -->
    
     - スケーラビリティ：PBFT の効率は、ネットワーク内のノードの数に強く依存します。ノード数が増加すると、必要なメッセージの数が指数関数的に増加し、ネットワークの遅延と帯域幅の使用が大幅に増加します。これは、大規模なネットワークでの使用を困難にします。 - vertual machine software にアクセスした 1 人が（承認されれば）ノードをいくらでも増やせるようになっていた
...
<!-- TODO 要出典．今使われてないならカットしてしまってもいいかも． -->
...
- トランザクション手数料の意義
  ...
  <!-- TODO 必要な情報はできる限り省略しないようにしてください．以下疑問点です．

  そもそもマイナーにはブロックを小さく保とうとする明確な動機が存在する
  
  (ブロックにトランザクションを追加すればするほど、ブロックを伝播する時間が長くなり
  → ブロックの容量が大きくなるためネットワーク上伝搬する際に不利ということ？Txは一つ一つサイズが異なるので上の文だけだと不明瞭です．
  
  、他のマイナーによって次のブロックが先に見つかるリスクが増え
  → 他のマイナーが別のブロックのマイニングに成功する，ということが言いたいのか，フルノードが他のマイナーが成功したブロックが伝搬されてくるのが早くてそっちのほうが取り込まれやすい，ということが言いたいのかどちらでしょう：
  
  、ブロック間のマイニング競争で不利になる)
  → ブロック間ではなくマイナー間？
   -->
  - しかしながら、ブロックにトランザクションを追加するたびにマイナーが得る収入が増えるとすると、マイナーの競争力の低下を補うインセンティブが発生する
  <!-- TODO これも必要な情報を省略しないで記述をお願いします．
  「マイナーの競争力の低下を補うインセンティブ」はそのまま読むと，マイナーの行動において競争力の低下を補うためのインセンティブがマイナーが得る収入を増やすことによって発生する，と読めます． -->
...
<!-- TODO この辺も説明が冗長かつ不明瞭です．Tx手数料があることがマイナーが適切にTxをブロックに取り込むインセンティブになっている，ということを伝えたいのであればそれをストレートに表現してください．そしてそれは上の「ランザクション手数料の意義」で記述されているのでこの計算方法の欄にはもう書く必要はないと思いますが・・． -->

  - 手数料はマイナーがトランザクションをブロックに含める動機になる、故に高い手数料は早い処理速度に繋がる。
<!-- TODO 高いだけでは不十分で，「単位容量あたりの手数料が」だと思います． -->
...
<!-- TODO 以下も文章が冗長に感じます．
似たような内容はまとめて記述するようにし，同じような内容が点在されて記述されないようにしてください． -->
...
<!-- TODO これはP2PKHじゃなくてP2PKということを書きたいんだと思われますが，省略されているため伝わらないです． -->
...
<!-- TODO 要するにマイニング報酬とブロック報酬は何が違うんでしょう？上の表現だと両方，新規発行時にプロトコルから得られる報酬と，ブロック内に入れたTx手数料の合計の２つをあわせたものと読めます． -->
出典・参考
...
<!-- TODO 上記でマイニング報酬とブロック報酬の２種類の用語をせっかく定義したので，ここでも単に報酬と書くのではなくどちらかで表現してください． -->
- マイニング報酬
...
<!-- TODO このプロセスによりの前後で，飛躍があります．なぜ不正行為やダブルスペンディングを防げるのか説明を加えてください． -->
  - マイナーに、マイニングを通して、ビットコインネットワークのセキュリティと整合性を保つよう促すためのもの。
<!-- TODO ここの飛躍を感じます．なぜマイニングを通すとセキュリティと整合性が保たれるんでしょう？ -->
- 半減期に関連する経済用語
<!-- TODO 経済用語として記述しているにも関わらず，他のところで出てこないのでなぜ記述されているのか混乱します． -->
  - デフレーション効果
...
- 手数料
<!-- TODO この辺も説明が不十分で何を伝えたいのかがはっきりしません．言葉を補ってください． -->
...
<!-- TODO 文章が不明瞭です．
「その一つのインプットの前のトランザクションID」は，inputが含まれるTxのTxIDということ？
「その一つのインプットの前のトランザクションインデックス」は？
 -->
- Coinbase トランザクションのインプットはアンロッキングスクリプトの代わりに任意のデータを入れられる
<!-- TODO ここも正確に表現してください．
インプットのロッキングスクリプトに対するアンロッキングスクリプトのところに任意のデータを入れられるということだと思われますが・・．
 -->
  - Genesis ブロックの新聞見出し
<!-- TODO これも説明がないとなんのことかわかりません． -->
  - 2 バイト～ 100 バイト

- 最初にブロック高を入れ、そのあとは任意のデータ
<!-- TODO 最初にブロック高を入れるルールってありましたっけ？出典を明示してください． -->
...
<!-- TODO この逸話は，↑のGenesis ブロックの新聞の見出しのところにまとめて説明してしまうのがいいかも． -->
...
<!-- TODO 「ポインター」というとメモリポインタとの誤解が生じるので「使用する UTXO を含むトランザクションのトランザクションID」の方が良いかと． -->
  - アウトプットインデックス（4 バイト）: 使用される UTXO のインデックス番号。
<!-- TODO これも正確に書くと上記トランザクションハッシュで指定されるTxのアウトプットインデックス，とかになるかと． -->
...
<!-- TODO オプトアウトってなんでしょう？日本語でわかり易い言葉で置き換え可能なら置き換えをお願いします． -->
...
<!-- TODO 説明が不足しているため，詳細に記述してください．なにがどうなっているとトランザクションの置き換えが発生し，どういう対策を施すとそれが防げるのか．また，その置き換えが発生することでネットワークの混雑時にTxが確実に処理されないということがどういう現象として発生しているのか説明をお願いします． -->
