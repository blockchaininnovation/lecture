...
- [トランザクションのモデル](#トランザクションのモデル)
  - [UTXO の保管について](#utxo-の保管について)
- [Bitcoin Script](#bitcoin-script)
  - [特徴](#特徴)
    - [1. スタックベースの処理](#1-スタックベースの処理)
    - [2. チューリング不完全](#2-チューリング不完全)
    - [3. オペコードを使用する](#3-オペコードを使用する)
    - [4. ステートレス](#4-ステートレス)
  - [Bitcoin Script における処理の具体例](#bitcoin-script-における処理の具体例)
- [トランザクション の構造](#トランザクション-の構造)
  - [vin の中身](#vin-の中身)
    - [scriptSig（Unlocking Script）について](#scriptsigunlocking-scriptについて)
  - [vout の構造](#vout-の構造)
    - [scriptPubKey（Locking Script）について](#scriptpubkeylocking-scriptについて)
  - [タイムロックについて](#タイムロックについて)
    - [トランザクション単位のタイムロック](#トランザクション単位のタイムロック)
      - [\*ビットについて](#ビットについて)
      - [\*エンディアン（バイトオーダー）について](#エンディアンバイトオーダーについて)
    - [UTXO 単位のタイムロック](#utxo-単位のタイムロック)
...
- [トランザクションの種類](#トランザクションの種類)
  ...
  - [データアウトプット（OP\_RETURN）](#データアウトプットop_return)
  - [Pay-to-Script-Hash (P2SH)](#pay-to-script-hash-p2sh)
      - [状況](#状況)
      - [P2SH を使用する理由](#p2sh-を使用する理由)
      - [Script](#script)
      - [実行](#実行)
    - [P2SH の利点](#p2sh-の利点)
    - [P2SH の注意点](#p2sh-の注意点)
- [Coinbase トランザクション](#coinbase-トランザクション)
  - [Coinbase トランザクションの vin について](#coinbase-トランザクションの-vin-について)
  - [Coinbase トランザクションの承認について](#coinbase-トランザクションの承認について)
- [\*SegWit（Segregate Witness）](#segwitsegregate-witness)
  - [SegWit の導入理由について](#segwit-の導入理由について)
    - [トランザクションデータの削減（効率化）](#トランザクションデータの削減効率化)
    - [取引速度の向上](#取引速度の向上)
    - [トランザクション展性（Malleability）の克服](#トランザクション展性malleabilityの克服)
  - [ブロックウェイト（Block Weight）](#ブロックウェイトblock-weight)
  - [ブロックサイズについて](#ブロックサイズについて)
    - [非 SegWit トランザクションの場合](#非-segwit-トランザクションの場合)
    - [SegWit トランザクションの場合](#segwit-トランザクションの場合)
  - [データ構造について](#データ構造について)
  - [トランザクション方式について](#トランザクション方式について)
    - [Pay-to-witness-public-key-hash （P2WPKH）](#pay-to-witness-public-key-hash-p2wpkh)
    - [Pay-to-witness-script-hash（P2WSH）](#pay-to-witness-script-hashp2wsh)
- [まとめ](#まとめ)
